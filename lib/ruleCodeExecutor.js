...
class RuleCodeExecutor {
  ...
  Console(message) {
    message = dayjs().format("HH:mm:ss ") + message;
    this._console.push(message);
    while (this._console.length > 100)
      this._console.shift();
  ...
  async Run() {
    ...
    this._devices = runningContext.GetDevices();
      ...
      process.setUncaughtExceptionCaptureCallback((err) => {
        ...
        for (const stack of stacks)
          if (stack.fileName == 'virtual.js') {
            runningContext.ReportRuleCodeExecutorError(err);
            virtualerror = true;
            break;
          }
