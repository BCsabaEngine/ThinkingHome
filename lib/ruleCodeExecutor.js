...
class OnceADay {
  ...
  addOccurenceInternal(datetime = null) {
    ...
    if (!this._dailyoccurences.has(date)) this._dailyoccurences.set(date, 0)

    this._dailyoccurences.set(date, this._dailyoccurences.get(date) + 1)
  }

  get hasoccured() {
    const date = dayjs().format('YYYY-MM-DD')
    if (this._dailyoccurences.has(date)) return this._dailyoccurences.get(date)
    return 0
  }
