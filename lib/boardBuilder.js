...
class ItemsPanel extends Panel {
  ...
  BuildItemHtmls(mode) {
    ...
    const devices = runningContext.GetDevices();
    for (const itemkey of Object.keys(this.items)) {
      const item = this.items[itemkey];
      const itemparts = item.match(/^([a-z0-9_]*).([a-z0-9]*)(\([ ]*({[ :"'\-,a-z0-9]*}[ ]*)\))?$/);
      if (!itemparts)
        throw new Error(`Invalid device.entity: ${item}`);

      const devicename = itemparts[1];
      const entitycode = itemparts[2];
      const entityparams = itemparts.length > 4 ? itemparts[4] : null;
      let entityparamsobj = {};

      if (entityparams)
        try { eval(`entityparamsobj = ${entityparams}`); }
        catch (err) { throw new Error(`Invalid entity parameters: ${entityparams}`); }
...
class BoardBuilder {
  ...
  Build() {
    const columncount = Math.min(this.GetPanelNames().length, 3);
