...
class RunningContext {
  ...
  GetRuleCodeExecutorMessage() {
    if (this.rulecodeexecutorerror)
      return this.rulecodeexecutorerror.message;
    return null;
  }
  ...
  GetRuleCodeExecutorStack() {
    if (this.rulecodeexecutorerror) {
      ...
      for (let line of this.rulecodeexecutorerror.stack.split('\n')) {
        if (line.startsWith('virtual.js:'))
          line = line.replace('virtual.js:', 'At line ');
        if (line.includes('at new Script (vm.js') || line.includes('at Script.runInContext (vm') || line.includes('at virtual.js:'))
          return result.join('\n');
        else
          result.push(line);
      }
  ...
  GetRuleCodeExecutorConsoleLines() {
    if (this.rulecodeexecutor)
      return this.rulecodeexecutor.GetConsoleLines();
    return [];
  }
  ...
  GetPlatforms() {
    let result = [];
    for (const code of Object.keys(this.platforms))
      result.push(this.platforms[code]);
    return result;
  }
  GetDevices() {
    let result = [];
    for (const platform of this.GetPlatforms())
      result = result.concat(platform.devices);
  ...
  GetEntities() {
    ...
    for (const device of this.GetDevices())
      for (const entitykey of Object.keys(device.entities))
        result.push(device.entities[entitykey]);
    return result;
  }
  debouncers = new DebouncerList(150);
  PublishEntityEvent(event, entity, args) {
    ...
    this.debouncers.Add(`platform_${platformcode}`, () => { wss.BroadcastToChannel(`platform_${platformcode}`); })
    this.debouncers.Add(`device_${devicename}`, () => { wss.BroadcastToChannel(`device_${devicename}`); })

    wss.BroadcastToChannel('board', { device: devicename, entity: entitycode, value: entityvalue });
  }

  GetWarningOrErrorPlatformCount() {
    let i = 0;
    for (const key of Object.keys(this.platforms))
      if (this.platforms[key].GetStatusInfos().filter(si => si.warning || si.error).length)
        i++;
    return i;
  }
  ...
  async Start() {
    ...
    app.get('/', this.WebMainPage);
    app.get('/board-:id', this.WebMainPage);
    app.get('/suggestdevicename', this.WebSuggestDevicename);
    app.get('/platform', this.WebPlatformList.bind(this));
    app.post('/platform/enable', this.WebPlatformEnable.bind(this));
    app.post('/platform/disable', this.WebPlatformDisable.bind(this));
  ...
  async StartPlatform(platform) {
    ...
    app.use(`/platform/${platform.code}`, platformobj.approuter);
  }
  ...
  async StartEnabledPlatforms() {
    ...
    for (const platform of Platform.GetAvailablePlatforms())
      if (enabledplatforms.includes(platform.code))
        await this.StartPlatform(platform);

    for (const key of Object.keys(this.platforms))
      this.platforms[key].Tick(0);
  ...
  async WebMainPage(req, res, next) {
    ...
    let allboards = [];
    if (userid)
      allboards = await BoardModel.GetAllByUserSync(userid);
    else
      allboards = await BoardModel.GetAllSync();

    let board = null;
    let boardindex = -1;
    for (let i = 0; i < allboards.length; i++) if (allboards[i].Id == boardid) { board = allboards[i]; boardindex = i; }
    if (!board)
      for (let i = 0; i < allboards.length; i++) if (allboards[i].IsPrimary) { board = allboards[i]; boardindex = i; }
    if (!board)
      if (allboards.length) { board = allboards[0]; boardindex = 0; }
    ...
    let errorcontent = "";
    if (board)
      try { content = new BoardBuilder(board.Yaml).Build(); }
      catch (error) { errorcontent = error.message; }
  ...
  async WebSuggestDevicename(req, res, next) {
    let devicename = (req.query.devicename || '').toLowerCase();
    if (devicename)
      devicename = await DeviceModel.FindFirstAvailableNameSync(devicename);
  ...
  async WebPlatformList(req, res, next) {
    ...
    for (const platform of platforms) {
      ...
      platform.statusinfo = [];
      if (this.platforms.hasOwnProperty(platform.code)) {
