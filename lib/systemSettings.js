class SystemSettings {
    ...
    return SystemSettingModel.GetAll()
      .then(settingrows => {
        for (const settingrow of settingrows) {
          if (properties.includes(settingrow.Name)) { this[settingrow.Name] = settingrow.Value }
        }
  ...
  LazyStore() {
    ...
    this._timer = setTimeout(function () {
      ...
      const rows = []
      for (const property of Object.getOwnPropertyNames(this).filter(item => !item.startsWith('_'))) { rows.push({ Name: property, Value: this[property] }) }
  ...
  AdaptFromObject(obj) {
    const properties = Object.getOwnPropertyNames(this).filter(item => !(item.startsWith('_')))
