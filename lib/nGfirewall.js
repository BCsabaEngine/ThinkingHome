  ...
  check: function (settings) {
    ...
    const hook = function (request, response, next) {
      ...
      if (querystring) {
        ...
        for (i = 0; i < this.FwQueryStrings.length; i++)
          if (querystring.toLowerCase().match('^' + this.FwQueryStrings[i].toLowerCase() + '$')) {

            if (typeof settings.onFailRequest === "function")
              settings.onFailRequest(reqip, querystring, this.FwQueryStrings[i]);
      ...
      if (useragent)
        for (i = 0; i < this.FwUserAgent.length; i++)
          if (useragent.toLowerCase().match('^' + this.FwUserAgent[i].toLowerCase() + '$')) {

            if (typeof settings.onFailRequest === "function")
              settings.onFailRequest(reqip, useragent, this.FwUserAgent[i]);
      ...
      if (referer)
        for (i = 0; i < this.FwReferer.length; i++)
          if (referer.toLowerCase().match('^' + this.FwReferer[i].toLowerCase() + '$')) {

            if (typeof settings.onFailRequest === "function")
              settings.onFailRequest(reqip, referer, this.FwReferer[i]);

            return;
          };

      const method = request.method;
      for (i = 0; i < this.FwMethod.length; i++)
        if (method.toLowerCase().match('^' + this.FwMethod[i].toLowerCase() + '$')) {

          if (typeof settings.onFailRequest === "function")
            settings.onFailRequest(reqip, method, this.FwMethod[i]);
