...
class BackupBuilder {
  ...
  async CreateBackup(keepcount) {
    ...
    try {
      const databasedumpbuffer = child_process.spawnSync(
        'mysqldump',
        [
      ...
      if (databasedumpbuffer.error)
        throw new Error(databasedumpbuffer.error.toString());

      if (databasedumpbuffer.stderr && databasedumpbuffer.stderr.length)
        throw new Error(databasedumpbuffer.stderr.toString());

      if (databasedumpbuffer.stdout && databasedumpbuffer.stdout.length) {
        zip.addFile("database.sql", databasedumpbuffer.stdout);
        backuplog.add(`Dump database OK: ${databasedumpbuffer.stdout.length / 1024} kB`);
      }
    }
    catch (error) {
