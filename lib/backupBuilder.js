...
class BackupBuilder {
  ...
  async CreateBackup(keepcount) {
    ...
    for (const file of glob.sync(backupexfolder + '**/*')) {
      try {
        if (!fs.lstatSync(file).isDirectory()) {
          const shortfilename = file.substr(backupexfolder.length)
          const filename = path.basename(shortfilename)
          const directory = shortfilename.substr(0, shortfilename.length - filename.length)
          if (filename !== 'readme.md') {
            zip.addLocalFile(file, backupexfolder + directory, filename)
          }
        }
      } catch (error) {
        const filename = path.basename(file)
        const errormsg = error.message
        zip.addFile(backupexfolder + `!${filename}`, Buffer.alloc(errormsg.length, errormsg))
      }
    }
