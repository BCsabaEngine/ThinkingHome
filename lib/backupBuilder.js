...
class BackupBuilder {
  ...
  async CreateBackup(keepcount) {
    ...
    try {
      const databasedumpbuffer = child_process.spawnSync(`mysqldump -h${config.database.server} -P${config.database.port} -u${config.database.username} -p${config.database.password} ${config.database.schema}`)
      zip.addFile("database.sql", databasedumpbuffer);
      backuplog.add(`Dump database OK: ${databasedumpbuffer.length / 1024} kB`);
    }
    catch (error) {
