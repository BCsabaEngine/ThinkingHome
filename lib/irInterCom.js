...
const IrSender = require('../platforms/Ir/Devices/IrSender')
...
module.exports = {
  ...
  GetDevicesHandledBy(handlerdevice) {
    const result = []

    for (const device of global.runningContext.GetDevices()) {
      if (device instanceof IrSender) {
  ...
  SendIr(handlerdevice, ircode) {
    ...
    for (const sender of this.GetSenderDevices()) {
      if (sender.id === handlerdevice && sender.SendIr(ircode)) { any = true }
    }
