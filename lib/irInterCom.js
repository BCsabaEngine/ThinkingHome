...
module.exports = {
  ...
  SendIr(handlerdevice, ircode) {
    ...
    for (const sender of this.GetSenderDevices()) {
      if (sender.name === handlerdevice && sender.SendIr(ircode)) { any = true }
    }
    if (!any) { logger.warn(`[IR intercom] Cannot send code '${ircode}' by '${handlerdevice}'. Is any IR sender with this name?`) }
  },
