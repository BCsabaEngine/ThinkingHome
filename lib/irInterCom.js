...
module.exports = {
  ...
  GetReceiverDevices() {
    ...
    for (const device of runningContext.GetDevices())
      for (const IrReceiverClass of this.IrReceiverClasses)
        if (device instanceof IrReceiverClass)
          result.push(device);
  ...
  GetSenderDevices() {
    ...
    for (const device of runningContext.GetDevices())
      for (const IrSenderClass of this.IrSenderClasses)
        if (device instanceof IrSenderClass)
          result.push(device);
  ...
  SendIr(ircode) {
    ...
    for (const sender of this.GetSenderDevices())
      if (sender.SendIr(ircode))
        any = true;
    if (!any)
      logger.warn(`[IR intercom] Cannot send code '${ircode}'. Is any IR sender?`);
  },

  IrReceived(ircode) {
    if (runningContext.platforms) {
      const ir = runningContext.platforms.ir;
      if (!ir) {
      ...
      if (!delegated)
        logger.warn(`[IR intercom] No device found for code '${ircode}'`);
    }
  }
